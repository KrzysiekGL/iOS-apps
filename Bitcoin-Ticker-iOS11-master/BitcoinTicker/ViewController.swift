...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    let bitcoinPriceDataModel = BitcoinPriceDataModel()
...
//    }
    ...
    func getBitcoinPrice(url: String) {
        Alamofire.request(url, method: .get).responseJSON {
            response in
            if response.result.isSuccess {
                print("Succes: got the Bitcoin price in selcted currency")
...
//    }
    ...
    func updateBitcoinPrice(json : JSON) {
        ...
        if let priceResult = json["ask"].double {
            bitcoinPriceDataModel.bitcoinPrice = "\(currencySymbolArray[currentRow]) \(priceResult)"
        }
        else {
            bitcoinPriceDataModel.bitcoinPrice = "N/A"
        }
    ...
    func updateUIWithBitcoinData() {
        bitcoinPriceLabel.text = bitcoinPriceDataModel.bitcoinPrice
    }
